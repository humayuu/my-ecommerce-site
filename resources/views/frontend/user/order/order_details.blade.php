                                            ...
                                            <td class="col-md-1">
                                                <label for=""> ${{ $item->price }}
                                                    @if ($order->status == 'pending')
                                                        <strong><span class="=badge badge-pill badge-success"
                                                                style="background-color: #418DB9">No File</span></strong>
                                                    @elseif($order->status == 'confirm')
                                                        <a target="_blank"
                                                            href="{{ asset('upload/pdf/' . $file->digital_file) }}">
                                                            <strong><span class="=badge badge-pill badge-success"
                                                                    style="background-color: #FF0000">Download
                                                                    Ready</span></strong></a>
                                                    @endif
                                                </label>
                                            </td>
                    ...
                    @if ($order)
                        <form method="POST" action="{{ route('return-order', $order->id) }}">
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control" cols="30" rows="05" placeholder="Return Reason" name="return_reason"
                                    id=""></textarea>
                            </div>
                            <button type="submit" class="btn btn-lg btn-danger">Submit</button>
                        </form>
                    @else
                        <span class="badge badge-pill badge-warning" style="background: red">You Have send this return
                            request for this Product.</span>
                    @endif;
                @endif
                <br><br>
